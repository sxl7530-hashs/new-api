...
export default function SettingsSidebarModulesAdmin(props) {
  ...
  async function onSubmit() {
    setLoading(true);
    const payload = sanitizeSidebarModulesConfig(sidebarModulesAdmin);
    try {
      ...
      if (success) {
        ...
        statusDispatch({
          ...
          payload: {
            ...statusState.status,
            SidebarModulesAdmin: JSON.stringify(payload),
          },
        });
  ...
  useEffect(() => {
    ...
    if (props.options && props.options.SidebarModulesAdmin) {
      try {
        ...
        setSidebarModulesAdmin(sanitizeSidebarModulesConfig(modules));
      } catch (error) {
        // 使用默认配置
        setSidebarModulesAdmin(getDefaultSidebarModulesAdmin());
      }
