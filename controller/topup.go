...
func EpayNotify(c *gin.Context) {
	...
	topUp, quotaToAdd, err := model.CompleteTopUpByTradeNo(verifyInfo.ServiceTradeNo, verifyInfo.Type)
	if err != nil {
		_, writerErr := c.Writer.WriteString("fail")
		if writerErr != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 webhook 响应写入失败 trade_no=%s client_ip=%s error=%q", verifyInfo.ServiceTradeNo, c.ClientIP(), writerErr.Error()))
		}
		switch err {
		...
		case model.ErrPaymentMethodMismatch:
			logger.LogWarn(c.Request.Context(), fmt.Sprintf("易支付 订单支付方式不匹配 trade_no=%s callback_type=%s client_ip=%s verify_info=%q", verifyInfo.ServiceTradeNo, verifyInfo.Type, c.ClientIP(), common.GetJsonString(verifyInfo)))
		case model.ErrTopUpStatusInvalid:
			logger.LogWarn(c.Request.Context(), fmt.Sprintf("易支付 订单状态不允许回调 trade_no=%s callback_type=%s client_ip=%s verify_info=%q", verifyInfo.ServiceTradeNo, verifyInfo.Type, c.ClientIP(), common.GetJsonString(verifyInfo)))
		default:
			logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 完成订单失败 trade_no=%s callback_type=%s client_ip=%s error=%q verify_info=%q", verifyInfo.ServiceTradeNo, verifyInfo.Type, c.ClientIP(), err.Error(), common.GetJsonString(verifyInfo)))
		}
		return
	}

	if quotaToAdd == 0 {
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("易支付 重复通知 trade_no=%s user_id=%d callback_type=%s client_ip=%s", verifyInfo.ServiceTradeNo, topUp.UserId, verifyInfo.Type, c.ClientIP()))
	} else {
		model.RecordTopupLog(topUp.UserId, fmt.Sprintf("使用在线充值成功，充值金额: %v，支付金额：%f", logger.LogQuota(quotaToAdd), topUp.Money), c.ClientIP(), topUp.PaymentMethod, "epay")
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("易支付 充值成功 trade_no=%s user_id=%d client_ip=%s quota_to_add=%d money=%.2f topup=%q", topUp.TradeNo, topUp.UserId, c.ClientIP(), quotaToAdd, topUp.Money, common.GetJsonString(topUp)))
	}

	_, writerErr := c.Writer.WriteString("success")
	if writerErr != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 webhook 响应写入失败 trade_no=%s client_ip=%s error=%q", verifyInfo.ServiceTradeNo, c.ClientIP(), writerErr.Error()))
	}
}
