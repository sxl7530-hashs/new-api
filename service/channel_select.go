...
func CacheGetRandomSatisfiedChannel(param *RetryParam) (*model.Channel, string, error) {
	...
	orderedGroups, resolveErr := ResolveOrderedTokenGroups(userGroup, param.TokenGroup)
	if resolveErr != nil {
		return nil, selectGroup, resolveErr
	}
	if len(orderedGroups) > 1 || model.IsAutoTokenGroup(param.TokenGroup) {
		if model.IsAutoTokenGroup(param.TokenGroup) && len(setting.GetAutoGroups()) == 0 {
		...
		for i := startGroupIndex; i < len(orderedGroups); i++ {
			...
			if model.IsAutoTokenGroup(param.TokenGroup) {
			...
			logger.LogDebug(param.Ctx, "Selected group: %s", currentGroup)
